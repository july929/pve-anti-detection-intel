                                      ...
                                      Error **errp)
{
    ...
    t->processor_family = 0xB3; /* Other use Processor Family 2 field */ //李晓流 dds666 modify 0xC6代表 Intel® Core™ i7 processor
    ...
    t->voltage = 0x8B;
    t->external_clock = cpu_to_le16(100); /* 100mhz */
    t->max_speed = cpu_to_le16(type4.max_speed); 
    t->current_speed = cpu_to_le16(type4.current_speed);
    t->status = 0x61; /* Socket populated, CPU enabled */
    t->processor_upgrade = 0x21; /* Other */
    t->l1_cache_handle = cpu_to_le16(0xFFFF); /* N/A */
    t->l2_cache_handle = cpu_to_le16(0xFFFF); /* N/A */
    t->l3_cache_handle = cpu_to_le16(0xFFFF); /* N/A */
    SMBIOS_TABLE_SET_STR(4, serial_number_str, type4.serial); //李晓流 dds666
    SMBIOS_TABLE_SET_STR(4, asset_tag_number_str, type4.asset); //李晓流 dds666
    SMBIOS_TABLE_SET_STR(4, part_number_str, type4.part); //李晓流 dds666
    ...
    t->processor_characteristics = cpu_to_le16(
        (1 << 2) | /* 64-bit Capable */
        (1 << 3) | /* Multi-Core */
        (1 << 4) | /* Hardware Thread */
        (1 << 7)   /* Execute Protection */
    );
    t->processor_family2   = cpu_to_le16(0xB3); //Other 李晓流 dds666 modify 和t->processor_family保持一致不一致都可以
...
static void smbios_build_type_17_table(unsigned instance, uint64_t size)
{
    ...
    snprintf(loc_str, sizeof(loc_str), "ChannelA-DIMM%d", instance);
    SMBIOS_TABLE_SET_STR(17, device_locator_str, loc_str);
    /* snprintf(loc_str, sizeof(loc_str), "%s %d", type17.loc_pfx, instance); */
    /* SMBIOS_TABLE_SET_STR(17, device_locator_str, "ChannelA-DIMM0");  */
    snprintf(loc_str, sizeof(loc_str), "BANK %d", instance);
    SMBIOS_TABLE_SET_STR(17, bank_locator_str, loc_str);
    /* SMBIOS_TABLE_SET_STR(17, bank_locator_str, "BANK 0");  */
     t->memory_type = 0x1A; /* DDR4 */  //李晓流 dds666 modify ddr4类型
    t->type_detail = cpu_to_le16(0x1080); /* test 0x80*/ //李晓流 dds666 modify 0x80代表 Synchronous
    t->speed = cpu_to_le16(3200); //李晓流 dds666 modify 3200mhz
    SMBIOS_TABLE_SET_STR(17, manufacturer_str, type17.manufacturer); //李晓流 dds666 modify
    ...
    SMBIOS_TABLE_SET_STR(17, part_number_str, type17.part);//李晓流 dds666 modify
    t->attributes = 0x02; /* 0x01 单根 0x02 双根 */ //李晓流 dds666 modify 1代表 记不得了，你要测一下
                       ...
                       Error **errp)
{
	...
	*/
	...
	smbios_build_type_7_table(0,"L1 Cache",0x180,cores_per_socket*0x20,0x4,0x5,0x7); //李晓流 dds666 added 设置1级Unified缓存，每个核心32kb
	smbios_build_type_7_table(1,"L2 Cache",0x181,cores_per_socket*0x200,0x5,0x5,0x8); //李晓流 dds666 added 设置2级Unified缓存，每个核心512kb
	smbios_build_type_7_table(2,"L3 Cache",0x182,0x4000,0x6,0x5,0x8); //李晓流 dds666 added 设置3级Unified缓存，16M
...
#define GET_DIMM_SZ ((i < dimm_cnt - 1) ? mc->smbios_memory_device_size \
	...
	smbios_build_type_28_table(1,"LM78A",0x67);//李晓流 dds666 added 0x7=motherboard
